...
Gestures:
  ...
  Praise_the_sun (two hands high above head)               -> memes/two hands on head .jpg
  go_right (right hand open, fingers pointing right)       -> memes/door_locked.jpg
...
GESTURE_MEMES = {
    "default": ["main_room.jpg"],
    "Praise_the_sun": ["praise_the_sun.jpg"],
    "spinCat": ["spin cat.mov"],
    "point_forward": ["floor_colapse.jpg"],
    "go_right": ["door_locked.jpg"],
}
...
def classify_hand(landmarks):
    ...
    # the 2D (x,y) projected distance between MCP (pts[5]) and TIP (pts[8])
    # appears much shorter than the real finger length. We normalize by
    # hand_scale so it's camera-distance independent.
    # pts are numpy [x, y, z]; z is depth (more negative = closer to camera).
    idx_2d_dist = float(np.hypot(pts[8][0] - pts[5][0], pts[8][1] - pts[5][1])) / hand_scale
    idx_z_delta = float(pts[5][2] - pts[8][2])  # positive = tip closer to camera
    index_pointing_at_camera = idx_2d_dist < 0.35 and idx_z_delta > 0.03
    ...
    return {
        ...
        "wrist": pts[0],
        "palmCenter": pts[9],
        "indexPointingAtCamera": index_pointing_at_camera,
    }
    ...
    def decide(self, hand_result):
        ...
        if len(hands) == 2:
            if face_is_fresh:
                ...
                near_face = all(
                    dist(h["palmCenter"], mouth_center) / face_width < 3.5 for h in hands
                )
                if near_face:
                    head_top_y = mouth_center[1] - face_width * 1.47
        ...
        # the camera. Detected using foreshortening (2D index tip-to-MCP is
        # short) + z-delta (tip closer to camera than knuckle).
        if (h["indexUp"] and not h["middleUp"] and not h["ringUp"]
                and not h["pinkyUp"] and h["indexPointingAtCamera"]):
            handedness = hand_result.handedness
            is_right = (
                handedness
                and len(handedness) > 0
                and len(handedness[0]) > 0
                and handedness[0][0].category_name == "Right"
            )
            if is_right:
        ...
        if h["curledCount"] == 0:
            vx = float(h["middleTip"][0] - h["wrist"][0])
            vy = float(h["middleTip"][1] - h["wrist"][1])
            is_horizontal = abs(vx) > 1.2 * abs(vy)
            pointing_right = vx > 0.15 * h["handScale"]
            thumb_vec = h["thumbTip"] - h["thumbBase"]
            finger_vec = h["middleTip"] - h["palmCenter"]
            thumb_parallel = angle_deg(thumb_vec, finger_vec) < 42 and not h["thumbOut"]
            ...
            is_right = (
                ...
                and len(handedness[0]) > 0
                and handedness[0][0].category_name == "Right"
            ) or (h["thumbTip"][1] < h["pinkyTip"][1])

            if is_horizontal and pointing_right and thumb_parallel and is_right:
                return "go_right"
    ...
    try:
        while True:
            ...
            if active_action is not None:
                if now - action_start_time >= ACTION_DISPLAY_MS:
                    active_action = None
                    action_end_time = now
            ...
            if is_cooling_down and (now - action_end_time >= ACTION_COOLDOWN_MS):
                is_cooling_down = False

            # 3. Process candidate gesture once stable
            if candidate_streak >= STABLE_FRAMES_REQUIRED:
                stable_gesture = candidate_gesture

                if stable_gesture == "default":
                    if not is_cooling_down:
                        last_triggered_gesture = None
                    if active_action is None and current_gesture != "default":
                        current_gesture = "default"
                        current_meme = random.choice(memes["default"])
                else:
                    in_cooldown = is_cooling_down and (stable_gesture == last_triggered_gesture)
                    already_active = (active_action is not None)
                    is_same_unreleased = (stable_gesture == last_triggered_gesture and not is_cooling_down and active_action is None)

                    if not already_active and not in_cooldown and not is_same_unreleased:
                        active_action = stable_gesture
                        last_triggered_gesture = stable_gesture
                        action_start_time = now
                        current_gesture = stable_gesture
                        if stable_gesture not in VIDEO_GESTURES:
                            current_meme = random.choice(memes[stable_gesture])
                        elif stable_gesture == "spinCat":
                            spin_video_cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
